 ...
 * [Full Complete Code - النسخة الشاملة مع إصلاح القوائم والرجوع + إغلاق التذكرة]
 */
...
// ============================================================
// [Zammad -> WhatsApp] - (المحافظة على الوسائط بالكامل + إغلاق التذكرة)
// ============================================================
...
    if (!phoneNumber) return res.status(200).send("No phone found");
    ...
    // ========================================
    // جديد: فحص حالة التذكرة (إغلاق التذكرة)
    // ========================================
    if (ticket.state === "closed") {
      console.log(`🔒 Ticket Closed for ${phoneNumber}`);
      
      // إرسال رسالة انتهاء التذكرة للزبون
      await sendWhatsApp({
        messaging_product: "whatsapp",
        to: phoneNumber,
        type: "text",
        text: { body: "✅ تم إغلاق التذكرة بنجاح.\n\nشكرًا لتواصلكم معنا. لأي استفسار جديد، يرجى إرسال أي رسالة للبدء من جديد." }
      });

      // تصفير الجلسة (عشان الرسالة الجاية تفتح القائمة من جديد)
