...
// =========================
const userTickets = {};
// الهيكل: { "رقم_المستخدم": { ticketId, welcomed } }
...
async function sendWhatsAppMessage(to, message, buttons = null) {
  try {
    ...
    if (buttons) {
      ...
      payload.interactive = { type: "button", body: { text: message }, action: { buttons } };
    }
...
app.post("/webhook", async (req, res) => {
  ...
  // =========================
  if (!userTickets[fromNumber]) userTickets[fromNumber] = { ticketId: null, welcomed: false };
  const userData = userTickets[fromNumber];

  // =========================
  // إرسال الترحيب مرة واحدة
  // =========================
  ...
  // =========================
  // التعامل مع زر "تواصل مع الدعم"
  // =========================
  if (messageText === "تواصل مع الدعم") {
    if (!userData.ticketId) {
      userData.ticketId = await createZammadTicket(fromNumber, messageText);
    }
    return;
  }

  // =========================
  // أي رسالة بعد إنشاء التذكرة تُضاف مباشرة
  // =========================
  if (userData.ticketId) {
    await appendToZammadTicket(userData.ticketId, messageText);
  }
});
